...
JWT_SECRET = os.environ['JWT_SECRET']  # P0: obligatoire, plus de fallback en clair
JWT_ALGORITHM = "HS256"
...
SUBSCRIPTION_PLANS = {
    ...
    "1week": {"name": "1 semaine", "price": 19.99, "price_cents": 1999, "duration_hours": 168, "max_rides_per_period": 15},
...
from routes.qr_tracking import router as qr_tracking_router
...
class UserRegister(BaseModel):
    ...
    # Comment as-tu connu Métro-Taxi ? (texte libre, facultatif)
...
async def get_current_user(request: Request) -> dict:
    ...
    # P0 fix 2026-09-10 (Séquence 2) — Révocation JWT côté serveur :
    # vérifier que le compte n'a pas été administrativement désactivé depuis
    # l'émission du token. `deactivated=True` bloque l'accès même avec un
    # JWT encore techniquement valide.
    # NB : is_active (chauffeur hors ligne) et is_validated (en attente) ne
    # sont PAS des motifs de blocage ici — ce sont des états opérationnels
    # gérés par le matching / le workflow de validation.
    role = payload.get("role")
    user_id = payload.get("user_id")
    if user_id and role in ("driver", "user"):
        collection = db.drivers if role == "driver" else db.users
        account = await collection.find_one(
            {"id": user_id},
            {"_id": 0, "deactivated": 1}
        )
        if account and account.get("deactivated") is True:
            raise HTTPException(status_code=401, detail="Compte désactivé — contactez le support")
...
async def register_user(data: UserRegister, request: Request):
    ...
    user_doc = {
        ...
        # Comment as-tu connu Métro-Taxi ? (texte libre, facultatif)
        "source_inscription": (data.source_inscription or "").strip() or None,
    }
...
async def register_driver(data: DriverRegisterWithRegion, request: Request):
    ...
    driver_doc = {
        ...
        "is_active": False,
        "is_validated": False,  # P0 fix 2026-09-09 — validation admin requise (fin auto-validation)
...
async def login(data: LoginRequest, request: Request):
    ...
    if driver and verify_password(data.password, driver["password"]):
        # Séquence 2b fix 2026-09-10 : compte administrativement désactivé → refus immédiat
        if driver.get("deactivated") is True:
            raise HTTPException(status_code=401, detail="Compte désactivé — contactez le support")
...
async def _maybe_send_transfer_voice_alert(driver_id: str, driver_lat: float, driver_lng: float) -> None:
    """Alerte vocale personnalisée transbordement (2026-09-14).
    Fire-and-forget : jamais bloquer l'update GPS driver.
    - Trouve la ride active du driver
    - Pour chaque transfer_point non encore alerté, calcule distance driver → point
    - Seuil L1 : < 500 m → 1ère alerte "prochain transbordement approche"
    - Seuil L2 : < 150 m → 2ème alerte "point de transbordement est proche"
    - Dedup : flag `voice_alerts_sent` en Mongo par (transfer_index, level)
    """
    try:
        from utils.helpers import calculate_distance as _dist
        ride = await db.ride_requests.find_one(
            {
                "driver_id": driver_id,
                "status": {"$in": ["accepted", "pickup", "in_progress"]},
            },
            {"_id": 0, "id": 1, "user_id": 1, "transfer_points": 1, "voice_alerts_sent": 1}
        )
        if not ride:
            return
        transfers = ride.get("transfer_points") or []
        if not transfers:
            return
        user = await db.users.find_one({"id": ride.get("user_id")}, {"_id": 0, "first_name": 1})
        if not user:
            return
        first_name = (user.get("first_name") or "").strip() or None
        sent = ride.get("voice_alerts_sent") or {}   # {"0": ["l1"], "1": ["l1","l2"]}

        for idx, tp in enumerate(transfers):
            tp_lat = tp.get("lat") or (tp.get("location") or {}).get("lat")
            tp_lng = tp.get("lng") or (tp.get("location") or {}).get("lng")
            if tp_lat is None or tp_lng is None:
                continue
            dist_km = _dist(driver_lat, driver_lng, tp_lat, tp_lng)
            dist_m = dist_km * 1000
            already = set(sent.get(str(idx), []))
            level = None
            if dist_m < 150 and "l2" not in already:
                level = "l2"
                body = (f"{first_name}, votre point de transbordement est proche."
                        if first_name else "Votre point de transbordement est proche.")
            elif dist_m < 500 and "l1" not in already:
                level = "l1"
                body = (f"{first_name}, votre prochain transbordement approche."
                        if first_name else "Votre prochain transbordement approche.")
            else:
                continue

            # Envoi push (fire-and-forget aussi)
            try:
                ...
                await send_push_notification(
                    ...
                    notification=NotificationPayload(
                        title="🚕 Métro-Taxi",
                        body=body,
                        data={"type": "transfer_approaching", "level": level, "first_name": first_name},
                    ),
                    user_type="user",
                )
            except Exception as _push_err:
                logging.getLogger(__name__).warning(f"[voice_alert] push failed: {_push_err}")

            # Dedup : marquer l'alerte comme envoyée
            already.add(level)
            sent[str(idx)] = sorted(already)
            await db.ride_requests.update_one(
                {"id": ride["id"]},
                {"$set": {"voice_alerts_sent": sent}}
            )
            # Une seule alerte par tick GPS (évite d'envoyer L1+L2 dans le même appel)
            break
    except Exception as e:
        logging.getLogger(__name__).warning(f"[voice_alert] check failed: {e}")
...
async def update_driver_location(data: LocationUpdate, current_user: dict = Depends(get_current_user)):
    ...
    # Alerte vocale personnalisée transbordement (fire-and-forget)
    asyncio.create_task(_maybe_send_transfer_voice_alert(driver_id, data.latitude, data.longitude))
